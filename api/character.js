...
4. "[${styleTrimmed} SPEC]" 섹션(하나 이상, 필요하면 여러 개로 나눠도 됨): 목표 스타일의 구조적 규칙을 항목별로 매우 구체적으로 기술하라 — 비율/등신, 시점, 얼굴 구조(눈·코·입 표현 방식), 헤어 표현 규칙, 신체·팔다리 비율, 채색·셰이딩 규칙, 그 세계관 특유의 필수 요소(있다면) 등. 실제로 그 스타일을 잘 아는 전문가가 쓴 것처럼 정확하고 디테일해야 한다.
   - 중요: 신체 비율·체형을 서술할 때 "2~3등신 SD", "통통한 몸", "짧고 통통한 팔다리" 같은 표현을 모든 스타일에 획일적으로 갖다 붙이지 마라. 이런 표현은 목표 스타일이 실제로 SD/치비(chibi) 계열일 때만 쓰고, 목표 스타일이 실제 작품에서 평범한 아동·성인 체형(예: 4~6등신)을 쓴다면 그 실제 비율을 반영하라. "${styleTrimmed}"라는 이름을 아는 사람이라면 바로 알아볼 만큼 그 작품/스타일 고유의 실제 그림체 특징(선화 두께, 눈코입 비례, 실제 신체 비율, 색감)을 정확히 재현하는 것이 목표이며, 근거 없이 캐릭터를 더 아이처럼·통통하게 만들지 마라.
